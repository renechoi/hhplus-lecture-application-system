...
#
#  Scenario: 이미 신청한 사용자가 다  시 신청 시도 - 실패
#    Given 사용자가 특강 신청 페이지에 접속하여 현재 제공되는 특강 목록을 조회한다
#    And 사용자가 "user2" 아이디로 특강 신청을 완료한다
#    When 사용자가 같은 "user2" 아이디로 다시 특강 신청을 시도하면
#    Then 특강 신청이 실패하여야 한다
#    And 실패 이유로 "이미 신청한 사용자" 메시지를 받아야 한다
#
